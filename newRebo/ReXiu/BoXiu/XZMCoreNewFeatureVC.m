...
- (void)layoutSubviews{
    ...
    [AppInfo shareInstance].pageControl.currentPage = cont;
...
- (UIButton *)vcPrepare{
    ...
    [AppInfo shareInstance].pageControl = [[UIPageControl alloc] initWithFrame:CGRectMake(0, 500, SCREEN_WIDTH, 30)];
    [AppInfo shareInstance].pageControl.pageIndicatorTintColor = [CommonFuction colorFromHexRGB:@"DED4CC"];
    [AppInfo shareInstance].pageControl.currentPageIndicatorTintColor = [CommonFuction colorFromHexRGB:@"9B8977"];
    [AppInfo shareInstance].pageControl.numberOfPages = 4;
    [AppInfo shareInstance].pageControl.currentPage = 0;
    [AppInfo shareInstance].pageControl.defersCurrentPageDisplay = YES;
    //添加
    [self.view addSubview:scrollView];
    [self.view addSubview:[AppInfo shareInstance].pageControl];
...
- (UIButton *)imageViewsPrepare{
    ...
    [self.imageNames enumerateObjectsUsingBlock:^(NSString *imageName, NSUInteger idx, BOOL *stop) {
        ...
        if(idx == self.imageNames.count - 2) {
