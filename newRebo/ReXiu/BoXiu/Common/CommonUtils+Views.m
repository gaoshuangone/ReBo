...
/*
 固定尺寸的button
 */
+(UIButton*)commonButNormalWithFrame:(CGRect)frame
                          withBounds:(CGSize)bounds
                         withOriginX:(CGFloat)x
                         withOriginY:(CGFloat)y
                isRelativeCoordinate:(BOOL)isRC withTarget:(id)target withAction:(SEL)action{
    
    UIButton* button = [CommonUtils commonButtonWithFrame:frame withTarget:target withAction:action];
    
    compl UIBarButtonItem ItemCount
